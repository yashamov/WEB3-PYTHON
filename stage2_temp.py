...
def mint_and_approve(YOUR_PRIVATE_KEY):
    ...
    gas_estimate = nft_contract.functions.mint().estimate_gas({
        'from': YOUR_ADDRESS,
        'gasPrice': gas_price,
    })
    ...
    # Подпись транзакции
    signed_transaction = opBNB_RPC.eth.account.sign_transaction(mint_tx, YOUR_PRIVATE_KEY)
    # Хэш
    transaction_hash = opBNB_RPC.eth.send_raw_transaction(signed_transaction.rawTransaction)
    print(f'Транзакция mint отправлена: {opBNB_RPC.to_hex(transaction_hash)}')
    ...
    SPENDER_ADDRESS = NFT_BRIDGE_CONTRACT_ADDRESS
    nonce = opBNB_RPC.eth.get_transaction_count(YOUR_ADDRESS)

    gas_estimate = nft_contract.functions.approve(SPENDER_ADDRESS, minted_token_id).estimate_gas({
        'from': YOUR_ADDRESS,
        'gasPrice': gas_price,
    })

    approve_tx = nft_contract.functions.approve(SPENDER_ADDRESS, minted_token_id).build_transaction({
        'from': YOUR_ADDRESS,
        'gas': gas_estimate,
        'gasPrice': gas_price,
        'nonce': nonce,
    ...
    signed_transaction = opBNB_RPC.eth.account.sign_transaction(approve_tx, YOUR_PRIVATE_KEY)
    tx_hash = opBNB_RPC.eth.send_raw_transaction(signed_transaction.rawTransaction)
    print(f'Транзакция approve отправлена: {opBNB_RPC.to_hex(tx_hash)}')
...
def transferNFT(YOUR_PRIVATE_KEY, token_id):
    ...
    gas_price = opBNB_RPC.eth.gas_price
    gas_estimate = nft_bridge_contract.functions.transferNFT(token_address, token_id, recipient_chain, recipient_bytes32, "0x0").estimate_gas({
        'from': YOUR_ADDRESS,
        'gasPrice': gas_price,
        'value': transfer_nft_amount
    })

    transfer_tx = nft_bridge_contract.functions.transferNFT(token_address, token_id, recipient_chain, recipient_bytes32, "0x0").build_transaction({
        'from': YOUR_ADDRESS,
        'gas': gas_estimate,
        'gasPrice': gas_price,
        'nonce': nonce,
