...
web3 = Web3(Web3.HTTPProvider("https://subnets.avax.network/defi-kingdoms/dfk-chain/rpc"))

# USDC contract address
usdc_contract_address = "0x3AD9DFE640E1A9Cc1D9B0948620820D975c3803a"

# Synapse Bridge contract address
# synapse_bridge_contract_address = "0x1c6ae197ff4bf7ba96c66c5fd64cb22450af9cc8"
synapse_bridge_contract_address = web3.toChecksumAddress('0x3C351E1afdd1b1BC44e931E12D4E05D6125eaeCa')
...
Jewel_address = '0xCCb93dABD71c8Dad03Fc4CE5559dC3D89F67a260'
...
for private_key in private_keys:
    ...
    if balance > 0:
        ...
        try:
            # Ваш код, вызывающий ошибку ContractLogicError
            gas_estimate = synapse_bridge_contract.functions.swapExactTokensForETH(balance, balance, #минимум получить
                                                                                      [usdc_contract_address, Jewel_address],
                                                                                      account_address, int(time.time()) + 600).estimateGas({
                'from': account_address,
                'gasPrice': 41932449048
            })
        except ContractLogicError as e:
        ...
        deposit_txn = synapse_bridge_contract.functions.swapExactTokensForETH(balance, balance, #минимум получить
                                                                                      [usdc_contract_address, Jewel_address],
                                                                                      account_address, int(time.time()) + 600).buildTransaction({
            'from': account_address,
            'gas': gas_estimate,
            'gasPrice': 41932449048,
            'nonce': nonce
