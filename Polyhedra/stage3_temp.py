...
opBNB_TOKEN_BRIDGE_CONTRACT_ADDRESS = '0x953a578c7Ce8F3A1BF625d182A8caf7181FD4BEB'
...
opBNB_TOKEN_BRIDGE_CONTRACT_ADDRESS = opBNB_RPC.to_checksum_address(opBNB_TOKEN_BRIDGE_CONTRACT_ADDRESS)
token_bridge_contract = opBNB_RPC.eth.contract(address=opBNB_TOKEN_BRIDGE_CONTRACT_ADDRESS, abi=TOKEN_BRIDGE_CONTRACT_ABI)


def bridge_to_bnb(YOUR_PRIVATE_KEY):
    ...
    amount = opBNB_RPC.to_wei(random.uniform(0.005, 0.006), 'ether')
    ...
    # Подпись транзакции
    signed_transaction = opBNB_RPC.eth.account.sign_transaction(bridge_tx, YOUR_PRIVATE_KEY)

    # Хэш
    transaction_hash = opBNB_RPC.eth.send_raw_transaction(signed_transaction.rawTransaction)
    print(f'Транзакция Bridge в BSC отправлена: {opBNB_RPC.to_hex(transaction_hash)}')
    ...
    print(f'Транзакция Bridge в BSC подтверждена в сети BSC')

    return YOUR_ADDRESS, opBNB_RPC.to_hex(transaction_hash)
...
for private_key in YOUR_PRIVATE_KEYS:
    try:
        address, transaction_hash = bridge_to_bnb(private_key)
